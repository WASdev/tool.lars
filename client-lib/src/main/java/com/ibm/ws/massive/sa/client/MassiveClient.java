...
import com.ibm.ws.massive.sa.client.model.WlpInformation.TypeLabel;
...
public class MassiveClient {
     ...
     */
    public MassiveClient(ClientLoginInfo loginInfo) {
        super();
     ...
     * @throws RequestFailureException
     */
    public List<Asset> findAssets(String searchString, TypeLabel assetType, Visibility visible) throws IOException, RequestFailureException {
     ...
     * @throws RequestFailureException
     */
    public void deleteAttachment(String assetId, String attachmentId)
     ...
     *
     * For basic auth credentials, both username *and* password
     * must be set in order for this to happen and it is an error for only one to
     * be set.
     *
    ...
    private void addAuthToConnection(HttpURLConnection connection) {
        ...
        if (loginInfo.getUserId() != null && loginInfo.getPassword() == null) {
            throw new AssertionError("userId is set but password is null. Either both or neither must be set.");
        }
        if (loginInfo.getUserId() == null && loginInfo.getPassword() != null) {
            throw new AssertionError("password is set but userId is null. Either both or neither must be set.");
        }
            ...
            (loginInfo.getProxy().getProxyPassword() != null))
        {
            ...
            String proxyBasicAuth = "Basic " + javax.xml.bind.DatatypeConverter.printBase64Binary(
     ...
     * @throws RequestFailureException
     */
    public Collection<Asset> getAssets(Type type) throws IOException, RequestFailureException {
     ...
     * @throws RequestFailureException
     */
    public Collection<Asset> getAssets(Collection<Type> types, Collection<String> productIds, Visibility visibility, Collection<String> productVersions) throws IOException, RequestFailureException {
     ...
     * @throws RequestFailureException
     */
    public Collection<Asset> getAssetsWithUnboundedMaxVersion(Collection<Type> types, Collection<String> rightProductIds, Visibility visibility) throws IOException, RequestFailureException {
    ...
    public enum FilterableAttribute {
        ...
        private FilterableAttribute(String attributeName) {
            this.attributeName = attributeName;
            this.secondaryAttributeName = null;
            this.valuesInSecondaryAttributeName = null;
        }

        private FilterableAttribute(String attributeName, String secondaryAttributeName, Collection<String> valuesInSecondaryAttributeName) {
            this.attributeName = attributeName;
     ...
     * @return
     */
    private boolean allFiltersAreEmpty(Map<FilterableAttribute, Collection<String>> filters) {
        for (Map.Entry<FilterableAttribute, Collection<String>> filter : filters.entrySet()) {
            Collection<String> values = filter.getValue();
            if (values != null && !values.isEmpty()) {
                return false;
     ...
     * @throws RequestFailureException
     */
    private Collection<Asset> getFilteredAssets(Collection<Type> types, Collection<String> productIds, Visibility visibility, Collection<String> productVersions,
    ...
    private void testResponseCode(HttpURLConnection connection) throws RequestFailureException, IOException {
        ...
        if (responseCode < 200 || responseCode >= 300) {
            ...
            if (errorStream != null) {
                ...
                // The content of the error stream coming from Massive seems to vary in structure (HTML, JSON) see if it is is JSON and get a good message or use it as is
    ...
    private String parseErrorObject(String errorObject) {
        ...
        try {
            ...
            Object errorMessage = json.get("message");
